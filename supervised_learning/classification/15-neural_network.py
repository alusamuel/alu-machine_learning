#!/usr/bin/env python3
"""Neural network with one hidden layer - training with verbose & graph"""
import numpy as np
...
class NeuralNetwork:
    ...
    def train(self, X, Y,
        ...
        # 0th iteration: state before any training
        A1, A2 = self.forward_prop(X)
        initial_cost = self.cost(Y, A2)
        if verbose:
            print("Cost after {} iterations: {}".format(0, initial_cost))
        if graph:
        ...
        for i in range(1, iterations + 1):
            # Use current A1, A2 to update params
            self.gradient_descent(X, Y, A1, A2, alpha)
            # Recompute forward propagation after update
            A1, A2 = self.forward_prop(X)
        ...
        # Plot after training if requested
        if graph:
